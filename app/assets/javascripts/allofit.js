function populateData(step) {
  var hashObject = {
    ...
    start_stop_name: step.transit_details.departure_stop.name,
    end_stop_name: step.transit_details.arrival_stop.name,
    end_latitude: step.end_location.lat,
    end_longitude: step.end_location.lng,
...
$(document).ready(function () {
  ...
  $('form').on('submit', function(e){
    ...
    $.get("https://maps.googleapis.com/maps/api/directions/json?alternatives=true&origin=" + start_location + "&destination=" + end_location + "&sensor=false&departure_time=" + departure_time + "&mode=transit", function(data) {
      var route = {
        steps: []
      }
      ...
      $.each(data.routes, function(i,v) {
        ...
        if (steps[0].travel_mode == "WALKING") {
          var step = steps[0];
          // var stepOne = populateData(step);
          ...
          route.steps.push(stepOne);
        } else if (steps[0].travel_mode == "TRANSIT") {
          var step = steps[0];
          var stepOne = populateData(step);
          // var stepOne = {
          //   travel_mode: "transit",
        ...
        if (steps[0].travel_mode == "WALKING" && steps[1].travel_mode == "TRANSIT"){
          var step = steps[1];
          var stepTwo = populateData(step);
          ...
          var getPredictionsURL = "http://webservices.nextbus.com/service/publicXMLFeed?command=predictions&a=sf-muni&r=";
            ...
            //               $('#whatever').append("G API " + agency + " lat " + start_latitude + " long " + start_longitude + " line short name " + line_short_name + " starting station " + start_stop_name);
